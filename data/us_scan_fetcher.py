...
def run_us_scan(fs=None, progress_cb: Optional[Callable] = None) -> tuple[bool, str]:
    ...
    try:
        # ① 股票清單
        _p("📋 從 NASDAQ FTP 取得全市場股票清單...")
        symbols = _get_us_symbols()
        _p(f"   取得 {len(symbols)} 支（過濾 ETF / 低流動性後）")

        if not symbols:
            return False, "無法從 NASDAQ FTP 取得股票清單"
...
def _get_us_symbols() -> list[str]:
    """
    從 NASDAQ FTP 下載全市場股票清單並過濾。
    - 排除 ETF、測試股
    - 只保留 1–5 個純英文字母的代號（過濾 warrant / preferred 等）
    - 限制 MAX_SYMBOLS 支
    """
    symbols: set[str] = set()
    ...
    try:
        ...
        syms = df[mask]["Symbol"].dropna().tolist()
        symbols.update(syms)
        logger.info(f"NASDAQ FTP: {len(syms)} 支")
    except Exception as e:
        logger.warning(f"NASDAQ FTP 失敗：{e}")
    ...
    try:
        ...
        mask = (
            ...
            (df.get("Exchange", pd.Series(["N"] * len(df))).isin(["A", "N", "P", "Q"]))
        )
        syms = df[mask][sym_col].dropna().tolist()
        symbols.update(syms)
        logger.info(f"NYSE/AMEX/ARCA FTP: {len(syms)} 支")
    except Exception as e:
        logger.warning(f"Other FTP 失敗：{e}")
